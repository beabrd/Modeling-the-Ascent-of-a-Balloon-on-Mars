...
def RK4_solver(t0,tf,h,y0):
    ...
    # store solutions where rows are time steps, and columns are [z,v]
...
radius_list = [4, 6, 8, 10, 12]  # meters
...
for r in radius_list:
    ...
    plt.plot(t, z, label=f"r_max = {r} m")

plt.xlabel("Time (s)")
plt.ylabel("Altitude z (m)")
plt.title("Effect of Maximum Balloon Radius on Ascent")
plt.legend()
plt.grid(True)
plt.show()
