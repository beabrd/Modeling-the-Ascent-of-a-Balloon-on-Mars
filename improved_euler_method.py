...
def derivatives(t,y):
    ...
    rho=mars_density(z)
    V= balloon_volume(z)
    A = balloon_cross_sectional_area(z)
    ...
    # calculate drag term
    F_drag= -0.5*rho*C_D*A*v*abs(v) # allows drag to be negative
...
def Improved_euler_solver(t0,tf,h,y0):
    #t0 is start time
    #tf is final time
    #h is time step
    ...
    # store solutions where rows are time steps, and columns is [z,v]
    ...
    for n in range(len(t_values)-1):
        ...
        y_predict=y_n + h*k1
        k2=derivatives(t_n,y_predict)


        y_values[n+1,:] = y_n + (h/2)*(k1+k2)
...
t2, y2 = Improved_euler_solver(t0,tf,h,y0)

z2 = np.maximum(y2[:, 0], 0.0)
v2=y2[:,1]
...
plt.title('Improved Euler Method: Altitude vs Time')
...
plt.title('Improved Euler Method: Velocity vs Time')
plt.grid(True) #Add Gridlines
plt.show()
